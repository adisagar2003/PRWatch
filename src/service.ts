...
export function launchdPlist(nodeBin: string, scriptPath: string, logsDirPath: string, pathEnv: string): string {
...
export function systemdUnit(nodeBin: string, scriptPath: string, pathEnv: string): string {
  return `[Unit]
...
export async function installService(): Promise<string> {
  ...
  if (process.platform === 'darwin') {
    ...
    await fs.writeFile(dest, launchdPlist(nodeBin, scriptPath, logsDir(), pathEnv));
  ...
  await fs.mkdir(path.dirname(dest), { recursive: true });
  await fs.writeFile(dest, systemdUnit(nodeBin, scriptPath, pathEnv));
